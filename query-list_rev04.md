# 001. Describe Omnibus Law (UU Cipta Kerja)

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT DISTINCT * WHERE {
  catapa:UUCiptaKerja ?p ?o .
}
```

# 002. Retrieve all articles (= pasal) of Omnibus Law

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT ?pasal ?text WHERE {
  ?pasal a catapa:Pasal .
  ?pasal catapa:partOf catapa:UUCiptaKerja .
  ?pasal catapa:text ?text
}
```

# 003. What is the textual content of Article (or Pasal) 5 Subsection (or Ayat) 1 of Omnibus Law?

### Pasal 5 memang tidak ada ayat 1?

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT * WHERE {
  ?x a catapa:Ayat .
  ?x catapa:noPasal 5 .
  ?x catapa:noAyat 1 .
  ?x catapa:partOf+ catapa:UUCiptaKerja .
  ?x catapa:text ?text .
}
```

# 004. Which are the articles of Chapter 2 (Bab 2) of Omnibus Law?

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT ?pasal ?text WHERE {
  ?bab a catapa:Bab .
  ?bab catapa:partOf catapa:UUCiptaKerja .
  ?bab catapa:noBab 2 .
  ?pasal a catapa:Pasal .
  ?pasal catapa:partOf ?bab .
  ?pasal catapa:text ?text .
}
```

# 005. Get subsections (= ayat) containing "kompensasi" and "buruh" that are added by Omnibus Law into other laws

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT ?ayat ?text WHERE {
  ?komponenUU catapa:partOf+ catapa:UUCiptaKerja .
  ?komponenUU catapa:hasilPenyisipan ?pasalYangDisisipkan .
  ?ayat catapa:partOf ?pasalYangDisisipkan .
  ?ayat catapa:text ?text .
  FILTER REGEX(str(?text), "kompensasi")
  FILTER REGEX(str(?text), "buruh")
}
```

# 006. Retrieve components of Omnibus Law that insert (= menyisipkan) articles (= pasal) into Labor Law (UU Ketenagakerjaan) and show the textual content of the articles

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT ?x ?pasalYangDisisipkan ?text WHERE {
  ?x catapa:partOf+ catapa:UUCiptaKerja .
  ?x catapa:menyisipkan catapa:UU-No-13-2003 .
  ?x catapa:hasilPenyisipan ?pasalYangDisisipkan .
  ?pasalYangDisisipkan catapa:text ?text .
}
```

# 007. Get components of Omnibus Law that amend (= mengubah) articles in Labor Law and compare the textual content of the old vs. new articles

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT ?x ?oldArticle ?oldText ?newArticle ?newText WHERE {
  ?x catapa:partOf+ catapa:UUCiptaKerja .
  ?x catapa:mengubah ?oldArticle .
  ?oldArticle catapa:partOf catapa:UU-No-13-2003 .
  ?oldArticle catapa:text ?oldText .
  ?x catapa:hasilPengubahan ?newArticle .
  ?newArticle catapa:text ?newText .
}
```

# 008. Give me components of Omnibus Law that remove (= menghapus) articles in Labor Law and show the textual content of the removed articles

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT * WHERE {
  ?x catapa:partOf+ catapa:UUCiptaKerja .
  ?x catapa:menghapus ?pasalYgDihapus .
  ?pasalYgDihapus catapa:partOf catapa:UU-No-13-2003 .
  ?pasalYgDihapus catapa:text ?text .
}
```

# 009. Retrieve implementing regulations (= peraturan pelaksana) of subsections (= ayat) in Labor Law that are removed by Omnibus Law

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT * WHERE {
  ?x catapa:partOf+ catapa:UUCiptaKerja .
  ?x catapa:menghapus ?pasalYgDihapus .
  ?pasalYgDihapus catapa:partOf catapa:UU-No-13-2003 .
  ?ayatPasalYgDihapus catapa:partOf ?pasalYgDihapus .
  ?peraturanPelaksana catapa:melaksanakanAyat ?ayatPasalYgDihapus .
}
```

# 010. How many are insertions, amendments, and removals of other laws in Omnibus Law?

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT ?type (COUNT(*) AS ?jumlah) WHERE {
  {
    ?x catapa:partOf+ catapa:UUCiptaKerja .
    ?x catapa:menyisipkan ?y .
    ?x a catapa:Angka .
    BIND("menyisipkan" AS ?type)
  }
  UNION
  {
    ?x catapa:partOf+ catapa:UUCiptaKerja .
    ?x catapa:mengubah ?y .
    ?x a catapa:Angka .
    BIND("mengubah" AS ?type)
  }
  UNION
  {
    ?x catapa:partOf+ catapa:UUCiptaKerja .
    ?x catapa:menghapus ?y .
    ?x a catapa:Angka .
    BIND("menghapus" AS ?type)
  }
} GROUP BY ?type
```

# 011. Get articles of Labor Law (UU Ketenagakerjaan) taking into account updates (= insertions/amendments/removals) from Omnibus Law (UU Cipta Kerja)

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT ?pasal WHERE {
  ?pasal catapa:partOf catapa:UU-No-13-2003 .
  ?pasal a catapa:Pasal .
  FILTER NOT EXISTS { ?pasalPenghapus catapa:menghapus ?pasal }
  FILTER NOT EXISTS { ?pasalPengubah catapa:mengubah ?pasal }
}
```

# 012. Get articles of Omnibus Law that are not about updating (= insertions/amendments/removals) other laws

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT ?pasal ?text WHERE {
  ?pasal a catapa:Pasal .
  ?pasal catapa:partOf catapa:UUCiptaKerja .
  ?pasal catapa:text ?text .
  FILTER NOT EXISTS { ?pasal catapa:mengubah ?y }
  FILTER NOT EXISTS {
    ?angka catapa:partOf ?pasal .
    { {?angka catapa:menyisipkan ?y} UNION {?angka catapa:mengubah ?y} UNION {?angka catapa:menghapus ?y} }
  }
}
```

# 013. Give me articles (= pasal) of Omnibus Law removing articles of laws legalized later than the year 2001

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT DISTINCT ?pasal WHERE {
  ?pasal a catapa:Pasal .
  ?pasal catapa:partOf catapa:UUCiptaKerja .
  ?angka catapa:partOf ?pasal .
  ?angka catapa:menghapus ?pasalDihapus .
  ?pasalDihapus catapa:partOf+ ?perundangan .
  ?perundangan a catapa:UndangUndang .
  ?perundangan catapa:year ?year .
  FILTER(?year > 2001)
}
```

# 014. Retrieve all subsections inserted by Omnibus Law into other laws and *optionally* the citations occurring in those subsections

```sparql
PREFIX catapa: <https://catapa.com/ns#>
PREFIX eli: <http://data.europa.eu/eli/ontology#>

SELECT ?ayat ?text ?citation WHERE {
  ?komponenUU catapa:partOf+ catapa:UUCiptaKerja .
  ?komponenUU catapa:hasilPenyisipan ?pasalYangDisisipkan .
  ?ayat catapa:partOf ?pasalYangDisisipkan .
  ?ayat catapa:text ?text .
  OPTIONAL {?ayat eli:cites ?citation}
}
```

# 015. Give me Omnibus Law articles amending another law where the amended law has an implementing regulation (= peraturan pelaksana) about Foreign Labors (Tenaga Kerja Asing/TKA)

```sparql
PREFIX catapa: <https://catapa.com/ns#>
PREFIX skos: <http://www.w3.org/2004/02/skos/core#>

SELECT * WHERE {
  ?pasal a catapa:Pasal .
  ?pasal catapa:partOf catapa:UUCiptaKerja .
  ?angka catapa:partOf ?pasal .
  ?angka catapa:mengubah ?pasalDiubah .
  ?pasalDiubah catapa:partOf+ ?perundanganYgPasalnyaDiubah .
  ?perundanganYgPasalnyaDiubah a catapa:UndangUndang .
  ?peraturanPelaksana catapa:melaksanakan ?perundanganYgPasalnyaDiubah .
  ?peraturanPelaksana catapa:subject/(skos:broader|skos:exactMatch)* catapa:TKA .
}
```

# 01. Obtain citations from newly inserted articles by Omnibus Law (UU Cipta Kerja) to the original version of Labor Law (UU Ketenagakerjaan)

```sparql
PREFIX catapa: <https://catapa.com/ns#>
PREFIX eli: <http://data.europa.eu/eli/ontology#>

SELECT ?ayatPengutip ?yangDikutip WHERE {
  ?angka catapa:partOf+ catapa:UUCiptaKerja .
  ?angka catapa:hasilPenyisipan ?pasalYangDisisipkan .
  ?ayatPengutip catapa:partOf ?pasalYangDisisipkan .
  ?ayatPengutip eli:cites ?yangDikutip .
  ?yangDikutip catapa:partOf+ catapa:UU-No-13-2003 .
  FILTER NOT EXISTS {
    ?yangDikutip catapa:partOf+ ?pasalYangDikutip .
    ?pasalYangDikutip a catapa:Pasal .
    ?angka catapa:partOf+ catapa:UUCiptaKerja .
    {{?angka catapa:hasilPenyisipan ?pasalYangDikutip} UNION {?angka catapa:hasilPengubahan ?pasalYangDikutip} UNION {?angka catapa:menghapus ?pasalYangDikutip}}
  }
}
```

# 016. Which law has the most number of updates (= insertions/amendments/removals) by Omnibus Law? For that law, show the most recent version taking into account the updates by Omnibus Law!

```sparql
PREFIX catapa: <https://catapa.com/ns#>

SELECT ?law ?pasal WHERE {

	{
		SELECT ?law (COUNT(*) AS ?numOfUpdates) WHERE {
			?x catapa:partOf+ catapa:UUCiptaKerja .
			{ {?x catapa:hasilPenyisipan ?y} UNION {?x catapa:mengubah ?y} UNION {?x catapa:menghapus ?y} }
			?x a catapa:Angka .
			?y a catapa:Pasal .
			?y catapa:partOf ?law .
		} GROUP BY ?law ORDER BY DESC(?numOfUpdates) LIMIT 1
	}

	?pasal catapa:partOf ?law .
	?pasal a catapa:Pasal .
	FILTER NOT EXISTS { ?pasalPenghapus catapa:menghapus ?pasal }
	FILTER NOT EXISTS { ?pasalPengubah catapa:mengubah ?pasal }

}
```
